...
This file preserves historical implementation and verification detail that was
previously mixed into `docs/CURRENT-DEV-STATE.md`. It is not current-state
authority. Use `docs/CURRENT-DEV-STATE.md` for present operational truth.
...
- Released production report version 6 (`c9e12e47-4735-4af9-97d5-bce036b16fcc`) for request `cd235456-12fe-4ef1-bc41-ca8891f5fae7` was first downloaded read-only and rerendered locally from its canonical 31-finding artifact. After deployment, the same fully reviewed request was regenerated and released through the existing versioning workflow as version 7 (`dccd50ff-f68b-4f9e-80bc-ac64e5003136`) without sending another delivery. The stored production PDF has a 9-page primary decision brief, all 31 findings and all 48 repair paths remain available, the technical appendix begins on page 10, and the complete PDF is 38 pages with 191 embedded public links.
...
- Production deployment `020a1bc7-f0e9-470c-82e2-3487455f26bc` serves commit `b177d3d` from `shelter-prep-phase1-dev`. `https://shelterprep.com/healthz` returned HTTP 200 from Railway with the expected Phase 1 health payload.
- The first live acceptance request, `a9ad3c6b-c632-46b5-af5c-fc891721184d`, failed before artifact persistence because pre-review local-professional extraction dereferenced an absent review-state map. Commit `b177d3d` made that boundary null-safe and added a fresh-unreviewed-artifact regression test. This failed request remains clearly labeled `failed` and separate from the corrected run.
- Corrected production request `cd235456-12fe-4ef1-bc41-ca8891f5fae7` completed with schema `shelter-prep-phase1-finding-investigation.v1`, 31 findings, 31 findings with independent research sources, 31 specific next tasks, zero paraphrase-only findings, and no fixture marker. It remains `needs_review` with 31 human decisions remaining; no finding was auto-approved or released.
- Production path pricing distribution is 5 paths with 3 sources, 25 with 2 sources, 17 with 1 source, and 1 explicitly blocked path with no defensible sourced range.
- Authenticated browser verification opened the exact corrected request in the production reviewer UI. The trip-hazard, electrical-panel-knockout, and HVAC-combustion-vent findings visibly showed independent research, corrected categories/trades, issue-specific repair paths, adjacent pricing-source counts/links, consequential unknowns, and concrete next tasks without paraphrase-only or internal benchmark language.
- Final verification: `npm test` passed 96/96, `npm run build` passed, the Python reasoning self-test passed, the exact real-report artifact passed server validation and the finding investigation audit, and `git diff --check` passed.
